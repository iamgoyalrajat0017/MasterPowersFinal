package com.masterpowers.masterpowers.earthbending.metal;
...
import com.masterpowers.masterpowers.ability.MetalAbility;
...
public class Extraction extends MetalAbility {
	...
	private int getAmount(int max) {
		final double roll = java.util.concurrent.ThreadLocalRandom.current().nextDouble(100);
		final int chanceMultiplier;
		if (roll < this.tripleChance) {
			chanceMultiplier = 2;
		} else if (roll < this.tripleChance + this.doubleChance) {
			chanceMultiplier = 1;
		} else {
			chanceMultiplier = 0;
		}
		final int min = chanceMultiplier * max + 1;
		return java.util.concurrent.ThreadLocalRandom.current().nextInt(max) + min;
	}
