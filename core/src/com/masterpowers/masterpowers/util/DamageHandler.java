...
import com.masterpowers.masterpowers.versions.legacy.LegacyDamageEventPasser;
import com.masterpowers.masterpowers.versions.modern.ModernDamageEventPasser;
...
import com.masterpowers.masterpowers.event.AbilityDamageEntityEvent;
...
public class DamageHandler {
	...
	public static double getIgnoreArmorPercentage(@NotNull final Ability ability) {
		FileConfiguration config = MasterPowers.plugin.getConfig();
	 ...
	 * We catch that {@link EntityDamageEvent} in {@link com.masterpowers.masterpowers.PKListener}, and if an instance of damage is up for "armor ignoring" (checked with {@link #ignoreArmor(Entity)}),
	...
	public static void setImmunity(final Entity entity, boolean immunity) {
		if (immunity) entity.setMetadata("BendingImmunity", new FixedMetadataValue(MasterPowers.plugin, true));
		else entity.removeMetadata("BendingImmunity", MasterPowers.plugin);
	}
}
