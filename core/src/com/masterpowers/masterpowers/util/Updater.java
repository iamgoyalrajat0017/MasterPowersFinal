		...
		if (this.checkUpdate) {
			this.runAsync(plugin, () -> {
				try {
					...
					this.urlc.setRequestProperty("User-Agent", "MasterPowers-UpdateChecker"); // GitHub requires a User-Agent.
					...
					try (BufferedReader reader = new BufferedReader(new InputStreamReader(this.urlc.getInputStream()))) {
						...
						// GitHub releases API returns JSON like {"tag_name":"1.12.0", ...}
						final String body = json.toString();
						final int tagIndex = body.indexOf("\"tag_name\"");
						if (tagIndex != -1) {
							final int firstQuote = body.indexOf('"', tagIndex + 11);
							final int secondQuote = body.indexOf('"', firstQuote + 1);
							if (firstQuote != -1 && secondQuote != -1) {
								this.updateVersion = body.substring(firstQuote + 1, secondQuote);
							}
						}
					} catch (final IOException e) {
						plugin.getLogger().info("Could not parse the latest release from GitHub.");
					}
				} catch (final IOException e) {
					plugin.getLogger().info("Could not connect to GitHub to check for updates.");
				}
				this.checkUpdate();
			});
		}
