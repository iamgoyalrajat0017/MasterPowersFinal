package com.masterpowers.masterpowers.util.logging;
...
public class LogFilter implements Filter {
	...
	public boolean isLoggable(final LogRecord record) {
		...
		if (record.getMessage() != null) {
			if (!record.getMessage().contains("MasterPowers")) {
				...
				if (!record.getThrown().getMessage().contains("MasterPowers")) {
					return false;
				}
				// record message doesnt have MasterPowers but throwable does.
			}
			...
			if (record.getThrown() != null) {
				...
				// record message null but throwable has MasterPowers.
				recordString = this.buildString(record);
			}
		}
		...
		if (!Bukkit.getServer().getPluginManager().isPluginEnabled(MasterPowers.plugin.getName())) {
			return false;
		}

		final String toRecord = recordString;
		Bukkit.getScheduler().runTaskLater(MasterPowers.plugin, (Runnable) () -> LogFilter.this.loggedRecords.add(toRecord), 10);
		return true;
	}
