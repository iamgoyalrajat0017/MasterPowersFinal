...
public class MovementHandler {
	...
	public void stopWithDuration(final long duration, final String message) {
		this.entity.setMetadata("movement:stop", new FixedMetadataValue(MasterPowers.plugin, this.ability));
		if (this.entity instanceof Player) {
			...
			this.runnable.runTaskTimer(MasterPowers.plugin, 0, 1);
		} else {
			this.runnable = new BukkitRunnable() {
			...
			new BukkitRunnable() {
				...
				public void run() {
					if (MovementHandler.this.entity.isOnGround()) {
						...
						this.cancel();
						MovementHandler.this.runnable.runTaskLater(MasterPowers.plugin, duration);
					}
				}
	...
	public void stop(final String message) {
		...
		if (this.entity instanceof Player) {
			...
			this.msg.runTaskTimer(MasterPowers.plugin, 0, 1);
		} else {
			new BukkitRunnable() {
	...
	public void reset() {
		...
		if (this.entity.hasMetadata("movement:stop")) {
			this.entity.removeMetadata("movement:stop", MasterPowers.plugin);
		}
	}
