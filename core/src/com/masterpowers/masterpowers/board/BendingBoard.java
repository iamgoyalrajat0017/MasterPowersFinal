package com.masterpowers.masterpowers.board;
...
public class BendingBoard {
	...
	public BendingBoard(final BendingPlayer bPlayer) {
		...
		// Permanent branding footer, always pinned to the very bottom of the board
		// (score -50 guarantees it stays below the 9 slots and any misc entries).
		Team brandTeam = bendingBoard.registerNewTeam("mp_brand");
		String brandEntry = "\u00A7r\u00A7k";
		brandTeam.addEntry(brandEntry);
		brandTeam.setPrefix("");
		brandTeam.setSuffix(ChatColor.translateAlternateColorCodes('&', "&8&m           &r &9&lMasterPowers&r &8&m           "));
		bendingSlots.getScore(brandEntry).setScore(-50);
	...
	private ChatColor getColor(String from, ChatColor def) {
		...
		try {
			...
			MasterPowers.plugin.getLogger().warning("Couldn't parse board color from '" + from + "', using default!");
			return def;
		}
	}
