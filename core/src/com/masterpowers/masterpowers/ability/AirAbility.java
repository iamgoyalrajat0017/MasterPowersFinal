...
public abstract class AirAbility extends ElementalAbility {
	...
	public static ParticleEffect getAirbendingParticles() {
		...
		try {
			...
			MasterPowers.log.warning("Your current value for 'Properties.Air.Particles' is not valid. Returning to the default SPELL particle.");
	...
	public static void playAirbendingSound(final Location loc) {
		if (getConfig().getBoolean("Properties.Air.PlaySound")) {
			...
			try {
				...
				MasterPowers.log.warning("Your current value for 'Properties.Air.Sound.Sound' is not valid.");
			} finally {
