package com.masterpowers.masterpowers.storage;
...
public class DBConnection {
	...
	public static void init() {
		...
		if (MasterPowers.plugin.getConfig().getString("Storage.engine").equalsIgnoreCase("mysql")) {
			sql = new MySQL(MasterPowers.log, host, port, user, pass, db, properties);
			if (((MySQL) sql).open() == null) {
				MasterPowers.log.severe("Disabling due to database error");
				GeneralMethods.stopPlugin();
				return;
			}
			isOpen = true;
			MasterPowers.log.info("Database connection established.");

			convertOldCooldownsTable();

			if (!sql.tableExists("pk_players")) {
				MasterPowers.log.info("Creating pk_players table");
				...
				try {
					...
					if (!md.getColumns(null, null, "pk_players", "subelement").next()) {
						MasterPowers.log.info("Updating Database with subelements...");
						...
						MasterPowers.log.info("Database Updated.");
					}
			...
			if (!sql.tableExists("pk_presets")) {
				MasterPowers.log.info("Creating pk_presets table");
			...
			if (!sql.tableExists("pk_cooldowns")) {
				MasterPowers.log.info("Creating pk_cooldowns table");
			...
			if (!sql.tableExists("pk_board")) {
				MasterPowers.log.info("Creating pk_board table");
			...
			if (!sql.tableExists("pk_temp_elements")) {
				MasterPowers.log.info("Creating pk_temp_elements table");
				final String query = "CREATE TABLE `pk_temp_elements` (uuid VARCHAR(36) NOT NULL, element VARCHAR(255) NOT NULL, expiry BIGINT);";
				sql.modifyQuery(query, false);
			}
		} else {
			sql = new SQLite(MasterPowers.log, "masterpowers.db", MasterPowers.plugin.getDataFolder().getAbsolutePath());
	...
	private static void convertOldCooldownsTable() {
		if (DBConnection.sql.tableExists("pk_cooldown_ids")) {
			...
			try {
				...
				MasterPowers.log.warning("Failed to get cooldown ids from database.");
				e.printStackTrace();
			}
			...
			try {
				...
				while (rs.next()) {
					...
					if (cooldownName == null || cooldownName.equals("")) {
						MasterPowers.log.warning("Failed to get cooldown name from database.");
						continue;
					}
				...
				MasterPowers.log.info("Converting old cooldowns to new cooldowns table... The DB will reconnect a few times.");
				...
				sql.getConnection().setAutoCommit(true);
				MasterPowers.log.info("Finished converting old cooldowns to new cooldowns table!");
			} catch (final SQLException e) {
				MasterPowers.log.warning("Failed to get cooldowns from database.");
				e.printStackTrace();
			}
		}
	}
