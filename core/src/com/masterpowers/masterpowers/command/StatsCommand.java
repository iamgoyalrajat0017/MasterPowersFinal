...
import com.masterpowers.masterpowers.util.Statistic;
...
public class StatsCommand extends PKCommand {
	...
	public void execute(final CommandSender sender, final List<String> args) {
		...
		if (containsGet) {
			Player target = null;
			if (args.size() == 4) {
				target = MasterPowers.plugin.getServer().getPlayer(args.get(3));
				if (target == null) {
			...
			}.runTaskAsynchronously(MasterPowers.plugin);
		}
	}
	...
	public List<String> getLeaderboard(final CommandSender sender, final Object object, final Statistic statistic, final int page) {
		...
		for (int index = minIndex; index < maxIndex; index++) {
			...
			final OfflinePlayer oPlayer = MasterPowers.plugin.getServer().getOfflinePlayer(uuid);
	...
	public List<UUID> pullUUIDs(final Statistic statistic, final Object object) {
		...
		for (final Player player : MasterPowers.plugin.getServer().getOnlinePlayers()) {
