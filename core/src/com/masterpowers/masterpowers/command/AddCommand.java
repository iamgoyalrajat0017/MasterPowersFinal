...
import com.masterpowers.masterpowers.event.PlayerChangeElementEvent.Result;
		...
		BendingPlayer.getOrLoadOfflineAsync(target).thenAccept(bPlayer -> {
			...
			if (element.equalsIgnoreCase("all")) {
						...
						if (online) {
							for (final SubElement sub : Element.getAllSubElements()) {
								if (bPlayer.hasElement(sub.getParentElement()) && ((BendingPlayer)bPlayer).hasSubElementPermission(sub)) {
									PlayerChangeSubElementEvent subEvent = new PlayerChangeSubElementEvent(sender, target, sub, com.masterpowers.masterpowers.event.PlayerChangeSubElementEvent.Result.ADD);
				...
				if (Arrays.asList(Element.getAllElements()).contains(e)) {
					...
					PlayerChangeSubElementEvent event = new PlayerChangeSubElementEvent(sender, target, sub, com.masterpowers.masterpowers.event.PlayerChangeSubElementEvent.Result.ADD);
					Bukkit.getServer().getPluginManager().callEvent(event);
