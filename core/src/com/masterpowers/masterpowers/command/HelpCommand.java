...
public class HelpCommand extends PKCommand {
	...
	public void execute(final CommandSender sender, final List<String> args) {
		...
		if (!this.hasPermission(sender) || !this.correctLength(sender, args.size(), 0, 1)) {
			...
			sender.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD + "MasterPowers " + ChatColor.RESET + ChatColor.GRAY + "\u2014 Official Bending Plugin (Night SMP Edition)");
			sender.sendMessage(ChatColor.YELLOW + "Choose an element (" + ChatColor.AQUA + "/mp choose <element>" + ChatColor.YELLOW + "), bind moves (" + ChatColor.AQUA + "/mp bind" + ChatColor.YELLOW + "), then use them to bend!");
			sender.sendMessage("");
			...
			sender.sendMessage("");
			sender.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD + "\u2605 CREATED BY MASTER! \u2605");
			return;
		}
